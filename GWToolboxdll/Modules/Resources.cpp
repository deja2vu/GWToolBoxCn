...
std::experimental::filesystem::path Resources::GetSettingsFolderPath()
{
    WCHAR path[MAX_PATH];
    SHGetFolderPathW(NULL, CSIDL_LOCAL_APPDATA | CSIDL_FLAG_CREATE, NULL, 0, path);
    return std::experimental::filesystem::path(path) / "GWToolboxCn";
}
std::experimental::filesystem::path Resources::GetPath(const std::experimental::filesystem::path& file)
{
    return GetSettingsFolderPath() / file;
}
std::experimental::filesystem::path Resources::GetPath(const std::experimental::filesystem::path& folder, const std::experimental::filesystem::path& file)
{
    return GetSettingsFolderPath() / folder / file;
}

void Resources::EnsureFolderExists(const std::experimental::filesystem::path& path)
{
...
bool Resources::Download(const std::experimental::filesystem::path& path_to_file, const std::wstring& url)
{
...
void Resources::Download(
    const std::experimental::filesystem::path& path_to_file, const std::wstring& url, std::function<void(bool)> callback)
{
...
void Resources::EnsureFileExists(
    const std::experimental::filesystem::path& path_to_file, std::function<void(bool)> callback)
{
...
void Resources::LoadTextureAsync(IDirect3DTexture9** texture, const std::experimental::filesystem::path& path_to_file)
{
    if (std::experimental::filesystem::exists(path_to_file)) {
...
void Resources::LoadTextureAsyncEx(
    IDirect3DTexture9** texture, const std::experimental::filesystem::path& path_to_file)
{
    EnsureFileExists(path_to_file, [this, path_to_file, texture](bool success) { 
        if (success && std::experimental::filesystem::exists(path_to_file)) {
            toload.push([path_to_file, texture](IDirect3DDevice9* device) {
                D3DXCreateTextureFromFileW(device, path_to_file.c_str(), texture);
            });
        }
...
void Resources::LoadTextureAsync(IDirect3DTexture9** texture, 
    const std::experimental::filesystem::path& path_to_file, WORD id)
{
