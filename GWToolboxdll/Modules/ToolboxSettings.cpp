...
void ToolboxSettings::DrawSettingInternal() {
    ...
    ImGui::Separator();
    DrawFreezeSetting();
    ImGui::Checkbox(u8"实时保存助手的配置", &save_location_data);
    ImGui::ShowHelp(u8"选中后，助手会每隔一秒钟执行一次保存配置的动作\n默认保存助手配置应该是点助手右下角的保存助手配置按钮.");
...
void ToolboxSettings::DrawFreezeSetting() {
    ImGui::Checkbox(u8"助手的所有界面变为可移动", &move_all);
    ImGui::ShowHelp(u8"选中后，你可以上下左右移动助手的所有界面,以及调整他们的大小");
}
