...
void MaterialsWindow::Draw(IDirect3DDevice9* pDevice) {
    ...
    if (ImGui::Begin(Name(), GetVisiblePtr(), GetWinFlags())) {
        ...
        if (ImGui::IsItemHovered()) ImGui::SetTooltip(u8"1 敏捷精华\n50 羽毛 和 50 闪耀之土");
        ImGui::SameLine();
        ...
        if (ImGui::Button(u8"点此询价##essence", ImVec2(100.0f, 0))) {
        ...
        ImGui::PopItemWidth();
        ImGui::SameLine();
        if (ImGui::Button(u8"买##essence", ImVec2(100.0f, 0))) {            
        ...
        if (ImGui::IsItemHovered()) ImGui::SetTooltip(u8"1 力量圣杯\n50 铁矿石 和 50 闪耀之土");
        ImGui::SameLine();
        x = ImGui::GetCursorPosX();
        y = ImGui::GetCursorPosY();
        ...
        FullConsPriceTooltip();
        ImGui::SameLine(ImGui::GetWindowWidth() - 100.0f - ImGui::GetStyle().WindowPadding.x);
        if (ImGui::Button(u8"点此询价##grail", ImVec2(100.0f, 0))) {
        ...
        ImGui::PopItemWidth();
        ImGui::SameLine();
        if (ImGui::Button(u8"买##grail", ImVec2(100.0f, 0))) {
        ...
        if (ImGui::IsItemHovered()) ImGui::SetTooltip(u8"1 救赎的防具\n50 铁矿石 和 50 骨头");
        ImGui::SameLine();
        x = ImGui::GetCursorPosX();
        y = ImGui::GetCursorPosY();
        ...
        FullConsPriceTooltip();
        ImGui::SameLine(ImGui::GetWindowWidth() - 100.0f - ImGui::GetStyle().WindowPadding.x);
        if (ImGui::Button(u8"点此询价##armor", ImVec2(100.0f, 0))) {
            EnqueueQuote(IronIngot);
            EnqueueQuote(Bone);
        }
        h = ImGui::GetCurrentWindow()->DC.LastItemRect.GetHeight();
        ...
        ImGui::PopItemWidth();
        ImGui::SameLine();
        if (ImGui::Button(u8"买##armor", ImVec2(100.0f, 0))) {
            int ironStock = GW::Items::CountItemByModelId(MaterialsWindow::GetModelID(IronIngot), stockStart, stockEnd);
        ...
        if (ImGui::IsItemHovered()) ImGui::SetTooltip(u8"1 勇气粉石\n100 花岗岩石板 和 100 闪耀之土");
        ImGui::SameLine();
        x = ImGui::GetCursorPosX();
        y = ImGui::GetCursorPosY();
        ImGui::Text(GetPrice(GraniteSlab, 10.0f, PileofGlitteringDust, 10.0f, 1000).c_str());
        ImGui::SameLine(ImGui::GetWindowWidth() - 100.0f - ImGui::GetStyle().WindowPadding.x);
        if (ImGui::Button(u8"点此询价##pstone", ImVec2(100.0f, 0))) {
        ...
        ImGui::PopItemWidth();
        ImGui::SameLine();
        if (ImGui::Button(u8"买##pstone", ImVec2(100.0f, 0))) {
        ...
        if (ImGui::IsItemHovered()) ImGui::SetTooltip(u8"1 复活卷轴\n25 植物纤维 和 25 骨头");
        ImGui::SameLine();
        x = ImGui::GetCursorPosX();
        y = ImGui::GetCursorPosY();
        ImGui::Text(GetPrice(PlantFiber, 2.5f, Bone, 2.5f, 250).c_str());
        ImGui::SameLine(ImGui::GetWindowWidth() - 100.0f - ImGui::GetStyle().WindowPadding.x);
        if (ImGui::Button(u8"点此询价##resscroll", ImVec2(100.0f, 0))) {
        ...
        ImGui::PopItemWidth();
        ImGui::SameLine();
        if (ImGui::Button(u8"买##resscroll", ImVec2(100.0f, 0))) {
        ...
        ImGui::SameLine();
        if (ImGui::Button(u8"买##common", ImVec2(50.0f - ImGui::GetStyle().ItemSpacing.x / 2, 0))) {
        ...
        ImGui::SameLine();
        if (ImGui::Button(u8"卖##common", ImVec2(50.0f - ImGui::GetStyle().ItemSpacing.x / 2, 0))) {
        ...
        ImGui::SameLine();
        if (ImGui::Button(u8"买##rare", ImVec2(50.0f - ImGui::GetStyle().ItemSpacing.x / 2, 0))) {
        ...
        ImGui::SameLine();
        if (ImGui::Button(u8"卖##rare", ImVec2(50.0f - ImGui::GetStyle().ItemSpacing.x / 2, 0))) {
        ...
        const char* status = "";
        if (cancelled) status = u8"已取消";
        else if (trans_done < trans_queued)
            status = u8"执行中...";
        else status = u8"就绪";
        ImGui::Text(u8"%s [%d / %d]", status, trans_done, trans_queued);
        ...
        ImGui::SameLine();
        if (ImGui::Button(u8"取消", ImVec2(100.0f, 0))) {
            Cancel();
        }
        if (ImGui::IsItemHovered()) ImGui::SetTooltip(u8"取消当前的查询操作");
    }
    ImGui::End();
}
...
std::string MaterialsWindow::GetPrice(MaterialsWindow::Material mat1, float fac1,
    ...
    if (p1 == PRICE_NOT_AVAILABLE || p2 == PRICE_NOT_AVAILABLE) {
        ...
        return u8"价格: （已发送请求，完成中...）";
    } else if (p1 == PRICE_COMPUTING_QUEUE || p2 == PRICE_COMPUTING_QUEUE) {
        return u8"价格: ( 已发送请求，查询中...)";
    } else {
        char buf[128];
        snprintf(buf, 128, u8"价格: %g k", (p1 * fac1 + p2 * fac2 + extra) / 1000.0f);
...
void MaterialsWindow::DrawSettingInternal() {
    ...
    ImGui::Checkbox(u8"金钱可自动存取[桑莱保险箱]", &manage_gold);
    ImGui::ShowHelp(u8"选中后，当购买材料时候，金钱不足时，会自动从仓库取\n当卖材料时，角色里的金钱已满，自动存入到仓库中");
    ImGui::Checkbox(u8"材料可自动存入[桑莱保险箱]", &use_stock);
    ImGui::ShowHelp(u8"选中后，当购买的材料无法存入角色栏的包包时，会自动存入到桑莱保险箱中");    
}
