...
void SettingsWindow::Draw(IDirect3DDevice9* pDevice) {
    ...
    if (ImGui::Begin(Name(), GetVisiblePtr(), GetWinFlags())) {
        ...
        */
        ...
        ImGui::SameLine();
        ImGui::Checkbox(u8"读图的时候，隐藏助手的设置界面", &hide_when_entering_explorable);

        ImGui::Text("General:");
        if (ImGui::CollapsingHeader(u8"助手操作手册")) {
        ...
        if (ImGui::Button(u8"保存助手配置", ImVec2(w, 0))) {
        ...
        ImGui::SameLine();
        if (ImGui::Button(u8"载入助手配置", ImVec2(w, 0))) {
