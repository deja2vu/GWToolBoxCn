...
void ToolboxUIElement::DrawSizeAndPositionSettings() {
    ...
    if (is_movable || is_resizable) {
        ...
        if (is_movable) {
            if(ImGui::DragFloat2(u8"位置坐标", reinterpret_cast<float *>(&pos), 1.0f, 0.0f, 0.0f, "%.0f"))
                ImGui::SetWindowPos(Name(), pos);
            ImGui::ShowHelp(buf);
        }
        if (is_resizable) {
            if(ImGui::DragFloat2(u8"界面大小", reinterpret_cast<float *>(&size), 1.0f, 0.0f, 0.0f, "%.0f"))
    ...
    if (is_movable) {
        ...
        new_line = true;
        ImGui::Checkbox(u8"不能移动界面的位置", &lock_move);
    }
    if (is_resizable) {
        if (new_line) ImGui::SameLine();
        new_line = true;
        ImGui::Checkbox(u8"不能调整界面的大小", &lock_size);
    }
    if (has_closebutton) {
        if (new_line) ImGui::SameLine();
        new_line = true;
        ImGui::Checkbox(u8"显示关闭按钮", &show_closebutton);
    }
    if (can_show_in_main_window) {
        if (new_line) ImGui::SameLine();
        new_line = true;
        if (ImGui::Checkbox(u8"添加到主菜单", &show_menubutton)) {
