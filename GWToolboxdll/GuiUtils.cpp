...
void GuiUtils::LoadFonts() {
    ...
    std::thread t([] {
        ...
        /*
        builder.AddChar(0x739B);//Âê
        builder.AddChar(0x6E0A);//Ô¨
        builder.AddChar(0x804A);//ÁÄ
        builder.AddChar(0x94AE);//Å¥
        builder.AddChar(0x83B1);//À³
        builder.AddChar(0x6C8C);//ãç
        builder.AddChar(0x9AB7);//÷¼
        builder.AddChar(0x9AC5);//÷Ã
        builder.AddChar(0x5B75);//·õ
        builder.AddChar(0x59C6);//Ä·
        builder.AddChar(0x61FC);//‘Ö
        builder.AddChar(0x5F17);//¸¥
        builder.AddChar(0x6D1B);//Âå
        builder.AddChar(0x7011);//ÆÙ
        builder.AddChar(0x59AE);//ÄÝ
        builder.AddChar(0x62F1);//¹°
        builder.AddChar(0x4FC4);//¶í
        builder.AddChar(0x97E9);//º«
        builder.AddChar(0x5A1C);//ÄÈ
        builder.AddChar(0x5C4F);//ÆÁ
        builder.AddChar(0x8000);//Ò«
        builder.AddChar(0x5C14);//¶û
        builder.AddChar(0x9CDE);//ÁÛ
        builder.AddChar(0x8910);//ºÖ
        builder.AddChar(0x4F0A);//ÒÁ
        builder.AddChar(0x72C4);//µÒ
        builder.AddChar(0x66DC);//ê×
        builder.AddChar(0x7459);//è§
        builder.AddChar(0x7425);//çú
        builder.AddChar(0x73C0);//çê
        builder.AddChar(0x8D4E);//Êê
        builder.AddChar(0x8F74);//Öá
       */
        builder.AddText(u8"º¥ÒÁ×ô×ÈÚ¤º¯Ää±°×¿ÅØÖäÏøÆ¡¿¦à¹¿²À¤ÐæÄÎÉÝÄÝÄ·ÄÈæß·õ¼Å¶ûá°áµÇÍáÔÎ¡³²Î×á¡ÓÄ\
            ±ÓÍ¢ß±¸¥ÃÖÑåí¬ê©êªÍØ¹°ÊïÂüÔý¼Ï¿ÂÎàèóÀâéªÇÁÌ´éæÕÓÂåÔ¨Õ¿ÆÙ×Æ¾ôêóµÒ²þâ±â²Ðþ\
            ÂêçêÉºçúÁÕÉªÅÏ»ûÎÁÁö²tÕèÁò»Ç¼ÀìõÄÂñ·ç²¼©¸¿ôäº²Ò®°¬Ü½ÎßÎ­Ì¦ÈãÀòÀ³ÈøµÙÞ§Þ±\
            Ñ¦òáòæòëó«×çÅµÌ·¼ÖºÕÖºåÈµÏÓô¿¤ÄÆÃúéFïíÍÓ°¯»ôÈÍÂ­ò¦æä÷¼÷ÃËè××¿ýô„öî÷¢ÁÛð°ðÕ÷ë\
            Ô¨ÁÄÅ¥À³ãç÷¼÷Ã·õÄ·‘Ö¸¥ÂåÆÙÄÝ¹°¶íº«ÄÈÆÁÒ«¶ûÁÛºÖÒÁµÒê×è§çúçêÊêÖáÏÀÉ®ÂÂìë");
